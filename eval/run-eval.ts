 ...
 */
const SUBSET = join(process.cwd(), process.env.EVAL_SUBSET ?? "eval/tasks/humaneval-systematic.jsonl");
const N = Number(process.env.EVAL_N ?? 30);
const BASELINE = process.env.EVAL_BASELINE ?? "gen-gptoss";
// Code spec only exercises `generators` + `tester`. Defaults are an all-FREE,
// fast ensemble (no deepseek/DeepThink on the critical path — it added ~2-3 min
// per problem as serial tester). deepseek stays the production anchor; override
// via EVAL_GENERATORS / EVAL_TESTER to benchmark other pools.
const POOLS: Pools = {
...
async function main(): Promise<void> {
  ...
  // Every "interesting" eval outcome (regression / capability-gap / recovery) is
  // distilled into an Obsidian learning so any future model reads it before acting.
